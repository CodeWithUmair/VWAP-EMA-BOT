...
- **`run_account.py` added at the repo root** — the Option C multi-account launcher
  from `MULTI-ACCOUNT.md`. New file, no bot code touched. Attaches to a *specific*
  MT5 terminal (`--mt5-path`/`--portable`), optional `--login/--password/--server`,
  per-account `--db`, `--dry-run`, all flags also from `.env`. ASCII prints (no
  `PYTHONUTF8` gotcha). Smoke-tested in `--dry-run` against the running terminal.
- **Still needs one manual step to go concurrent:** a *second* MT5 terminal install
  (separate folder) logged into one of the spare demo accounts
  **472611148 / 474438988 / 474438985**. Steps in
  `sessions/2026-09-02-multi-account-launcher.md` and `.env.example`.
- **Tests 14/14; backtest unchanged** (synthetic data, OOS −0.52R, noise gate
  FAILED, live runners bypass it on demo). No real-data backtest yet.
